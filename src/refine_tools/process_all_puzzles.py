...
from complete_mapper import CompleteRNAMapper
...
def process_selected_puzzles(puzzle_sets):
    ...
    for idx, puzzle_set in enumerate(puzzle_sets, 1):
        ...
        for model_idx, model_file in enumerate(model_files, 1):
            ...
            try:
                ...
                result = mapper.process_all(target_file, model_file, output_file)
                ...
                if residue_mapping:
                    print(f"OK (zmapowano {len(residue_mapping)} reszt, RMSD: {rmsd:.3f})")
                    success += 1
...
def main():
    ...
    # Sprawdź czy complete_mapper.py istnieje
    if not os.path.exists("complete_mapper.py"):
        print("BŁĄD: complete_mapper.py nie znaleziony!")
    ...
    process_selected_puzzles(selected_sets)
